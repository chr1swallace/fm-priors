...
![output](pp-sims.png)
*Effect of different prior choices on the distribution of posterior probabilities at the causal variant under different simulation structures.* We found the distributions were broadly similar across different priors for a range of simulated scenarios. Each distribution is represented by its mean (mid horizontal bar), its 10th and 90th centile (outer horizontal bars) and its 1st and 99th centile (limits of vertical line).  Generally, posterior probabilities are higher when the causal variant is in low LD with other variants in the region, when power is greaters (smaller P value) / larger sample sizes. In higher power situations (P < 10-8), the choice of prior distribution (Gaussian or Laplace) has very litted effect on the distribution of posterior probabilities at the causal variant,
